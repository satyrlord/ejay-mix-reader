...
- **TypeScript for browser and build tooling** — all `src/`, `scripts/`, and
  `tests/` code is `.ts`.
- **TypeScript for extraction tools** — `tools/pxd-parser.ts`,
  `tools/reorganize.ts`, `tools/find-duplicates.ts`, and
  `tools/rename-samples.ts` are batch operation scripts that run offline via
  `tsx`. They are not part of the browser runtime or build pipeline.
