...
function createAppController(app: HTMLElement): () => void {
  ...
  function collectMixAudioUrls(plan: MixPlaybackPlan): string[] {
    return [...new Set(
      plan.events
  ...
  function categoryTokenFromAudioUrl(audioUrl: string | null): string {
  ...
  function categoryColorFromAudioUrl(audioUrl: string | null): string {
  ...
  function timelineBpm(plan: MixPlaybackPlan): number {
  ...
  function clampMixStartBeat(beat: number): number {
    if (!activeMixPlan || activeMixPlan.loopBeats === null) return 0;
    const maxStartBeat = Math.max(0, activeMixPlan.loopBeats - 1);
  ...
  function refreshSamples(): void {
    ...
    if (!isAllEntry(state.productMode)) {
      const allowedProducts = new Set(state.productMode.productIds);
      baseSamples = state.samples.filter((sample) => {
    ...
    let filtered = sortSamplesByKey(
      filterSamples(baseSamples, filters),
      state.gridSortKey,
      state.gridSortDir,
    );
